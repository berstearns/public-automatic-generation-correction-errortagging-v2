# Pythia 1B fine-tuned on EFCAMDAT all-data — Error Profile Experiment

## Goal

Run the full generate-GEC-ERRANT-analysis pipeline using **ft-pythia-1b**
(Pythia 1B fine-tuned on EFCAMDAT all-data) on learner corpus data.
This model is a **fine-tuned artificial learner**: pre-trained EleutherAI/pythia-1b
...
3. Generate continuations with ft-pythia-1b (fine-tuned on EFCAMDAT)
...
- **ft-pythia-1b** (1B parameters)
- Base: `EleutherAI/pythia-1b` (pre-trained)
- Fine-tuned on: EFCAMDAT all-data (full learner corpus, all CEFR levels)
- Architecture family: pythia
- rclone source: `i:/<your-rclone-models>/pythia/pythia-1b-all-data`
