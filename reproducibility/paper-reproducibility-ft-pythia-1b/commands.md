...
```bash
rclone copy "i:/<your-rclone-models>/pythia/pythia-1b-all-data/final" \
    "./models/pythia/pythia-1b-all-data/final" \
    --progress
```

Verify:
```bash
ls ./models/pythia/pythia-1b-all-data/final/config.json
```

## One-command run (recommended)

```bash
.venv/bin/python reproducibility/paper-reproducibility-ft-pythia-1b/scripts/run_experiment.py
```
...
.venv/bin/python -m gen_gec_errant.pipeline \
    --config reproducibility/paper-reproducibility-ft-pythia-1b/experiment/configs/norm-CELVA-SP.yaml
```
