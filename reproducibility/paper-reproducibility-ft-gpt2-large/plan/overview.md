# GPT-2 Large (774M) fine-tuned on EFCAMDAT all-data — Error Profile Experiment

## Goal

Run the full generate-GEC-ERRANT-analysis pipeline using **ft-gpt2-large**
(GPT-2 Large (774M) fine-tuned on EFCAMDAT all-data) on learner corpus data.
This model is a **fine-tuned artificial learner**: pre-trained gpt2-large
...
3. Generate continuations with ft-gpt2-large (fine-tuned on EFCAMDAT)
...
- **ft-gpt2-large** (774M parameters)
- Base: `gpt2-large` (pre-trained)
- Fine-tuned on: EFCAMDAT all-data (full learner corpus, all CEFR levels)
- Architecture family: gpt2
- rclone source: `i:/<your-rclone-models>/gpt2/gpt2-large-all-data`
