...
```bash
rclone copy "i:/<your-rclone-models>/smollm2/smollm2-1.7b-all-data/final" \
    "./models/smollm2/smollm2-1.7b-all-data/final" \
    --progress
```

Verify:
```bash
ls ./models/smollm2/smollm2-1.7b-all-data/final/config.json
```

## One-command run (recommended)

```bash
.venv/bin/python reproducibility/paper-reproducibility-ft-smollm2-1.7b/scripts/run_experiment.py
```
...
.venv/bin/python -m gen_gec_errant.pipeline \
    --config reproducibility/paper-reproducibility-ft-smollm2-1.7b/experiment/configs/norm-CELVA-SP.yaml
```
