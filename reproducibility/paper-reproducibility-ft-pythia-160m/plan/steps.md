...
- Check if `./models/pythia/pythia-160m-all-data/final` exists locally
- If not: download from `i:/<your-rclone-models>/pythia/pythia-160m-all-data/final` via rclone
...
- For each dataset (CELVA-SP, EFCAMDAT-test, KUPA-KEYS):
  ...
  - Generate continuations with ft-pythia-160m from local checkpoint
